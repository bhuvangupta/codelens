<script lang="ts">
	...
	let providers: DomainProvider[] = $state([]);
	...
	onMount(async () => {
		...
		try {
			...
			if (res.ok) {
				...
				// Fallback to default Google if API fails
				providers = [{ providerId: 'google', displayName: 'Google', domain: null }];
			}
		} catch {
			// Fallback to default Google
			providers = [{ providerId: 'google', displayName: 'Google', domain: null }];
		} finally {
	...
	// Check if provider is Google-based (for icon)
	function isGoogleProvider(providerId: string): boolean {
		return providerId === 'google' || providerId.startsWith('google-');
	}
</script>
					...
					<div class="space-y-3">
						...
						{:else}
							{#each providers as provider}
								{#if isGoogleProvider(provider.providerId)}
									<!-- Google Login Button -->
									<a
										href={getLoginUrl(provider.providerId)}
										rel="external"
									...
									>
										...
										<span class="font-medium text-gray-700">Sign in with {provider.displayName}</span>
								...
								{:else if provider.providerId === 'github'}
									...
									>
										...
										<span class="font-medium text-white">Sign in with {provider.displayName}</span>
